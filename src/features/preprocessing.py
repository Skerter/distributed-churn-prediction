...
def fill_numeric_na(df: pd.DataFrame, strategy="median", target="CHURN") -> pd.DataFrame:
    ...
    for col in num_cols:
        if strategy == "median":
            ...
            logger.debug("Заполняем пропуски в числовом признаке '%s' медианой: %s", col, fill_value)
        elif strategy == "mean":
            fill_value = df[col].mean()
            logger.debug("Заполняем пропуски в числовом признаке '%s' средним: %s", col, fill_value)
        else:
